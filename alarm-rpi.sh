...
readonly MOUNTPOINT="/mnt/alarm-rpi"
readonly STAGING_DIR="/tmp/alarm-rpi"
...
__M.installer.setup () {
    ...
    if [[ "${DISK_DEV}" =~ [0-9]$ ]]; then
        DISK_PART_PREFIX="${DISK_DEV}p"
    else
        DISK_PART_PREFIX="${DISK_DEV}"
    fi
    DISK_PART_BOOT="${DISK_PART_PREFIX}1"
    DISK_PART_ROOT="${DISK_PART_PREFIX}2"
